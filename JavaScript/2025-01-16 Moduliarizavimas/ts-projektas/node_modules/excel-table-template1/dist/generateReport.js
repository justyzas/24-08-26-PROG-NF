...
function createTableHeaders(row, column, headers, sheet) {
...
function createTableData(data, keys, cell, worksheet) {
    data.forEach((saleData, rowIndex) => {
        //eina per duomenis (eilutes)
        keys.forEach((key, columnIndex) => {
            // eina per stulpelius
            const dataCell = worksheet.getCell(cell.fullAddress.row + rowIndex, cell.fullAddress.col + columnIndex);
...
export async function generateReport(data, options, headerMapOptions) {
    ...
    createTableHeaders(4, 1, headerMapOptions.map((option) => option.header), sheet); //stulpeliu pavadinimu eilutes kurimas
    const tableStartCell = sheet.getCell("A5");
    createTableData(data, headerMapOptions.map((option) => option.key), tableStartCell, sheet); //Visos lentelės sukūrimas
